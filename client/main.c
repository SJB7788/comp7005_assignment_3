...
static void parse_arguments(int argc, char *argv[], char **message, char **key, char **ip_address, char **port);
static in_port_t parse_port(const char *binary_name, const char *port_str);
_Noreturn static void usage(const char *program_name, int exit_code, const char *message);
static void convert_address(const char *address, struct sockaddr_storage *addr);
static int create_socket(int domain, int type, int protocol);
static void connect_socket(int sockfd, struct sockaddr_storage *addr, in_port_t port);
...
static int decrypt_vigenere_cipher(char *message, char *key, char buffer[], size_t buffer_size);
...
static int decrypt_vigenere_cipher(char *message, char *key, char buffer[], size_t buffer_size)
{
    ...
    for (size_t i = 0; i < message_len; i++)
    {
        ...
        if (isalpha(curr_char))
        {
            ...
            buffer[i] = shift_char(curr_char, -shift_val);
